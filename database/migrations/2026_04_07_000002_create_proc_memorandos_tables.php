...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('proc_memorandos', function (Blueprint $table) {
            ...
            $table->text('conteudo');
            $table->foreignId('remetente_id')->constrained('users', indexName: 'proc_memorandos_x_users_X_remetente_id');
            ...
            $table->foreignId('documento_id')->nullable()
                ->constrained('ged_documentos', indexName: 'proc_memorandos_x_ged_documentos_X_documento_id');
            $table->timestamps();
            $table->softDeletes();
        ...
        Schema::create('proc_memorando_destinatarios', function (Blueprint $table) {
            $table->id();
            $table->foreignId('memorando_id')->constrained('proc_memorandos', indexName: 'proc_memorando_destinatarios_x_proc_memorandos_X_memorando_id');
            $table->foreignId('usuario_id')->nullable()->constrained('users', indexName: 'proc_memorando_destinatarios_x_users_X_usuario_id');
        ...
        Schema::create('proc_memorando_anexos', function (Blueprint $table) {
            $table->id();
            $table->foreignId('memorando_id')->constrained('proc_memorandos', indexName: 'proc_memorando_anexos_x_proc_memorandos_X_memorando_id');
            $table->string('nome', 255);
            $table->string('arquivo_path', 500);
            $table->bigInteger('tamanho');
            $table->string('mime_type', 100);
            $table->foreignId('enviado_por')->constrained('users', indexName: 'proc_memorando_anexos_x_users_X_enviado_por');
            $table->timestamps();
        ...
        Schema::create('proc_memorando_respostas', function (Blueprint $table) {
            $table->id();
            $table->foreignId('memorando_id')->constrained('proc_memorandos', indexName: 'proc_memorando_respostas_x_proc_memorandos_X_memorando_id');
            $table->foreignId('usuario_id')->constrained('users', indexName: 'proc_memorando_respostas_x_users_X_usuario_id');
            $table->text('conteudo');
            $table->timestamps();
